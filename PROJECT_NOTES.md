...
**সর্বশেষ commit (২০২৬-০৮-১০):** Dark/Light থিম টগল যোগ করা হয়েছে।
...
ইংলিশে অনুবাদ করা হয়েছে (আইকনে "মি"→"M" সহ)। **নতুন কোনো ফিচার/UI-তে
বাংলা টেক্সট বা ফন্ট যোগ করা যাবে না** যদি না ইউজার আবার স্পষ্টভাবে চান
— কোড কমেন্ট ব্যতিক্রম, সেগুলো বাংলায় লেখা চলতে থাকবে।

> ⚠️ **`worker/worker.js`-এর ৩টা error মেসেজও ইংলিশ করা হয়েছিল** ("ভুল
> PIN" → "Incorrect PIN" ইত্যাদি) — এটা লাইভ করতে `wrangler deploy`
> লাগে (GitHub push যথেষ্ট না)। এখনো deploy হয়ে থাকলে এই নোটটা মুছে
> ফেলা যায়, না হলে ইউজারকে মনে করিয়ে দিতে হবে।
