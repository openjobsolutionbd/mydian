...
**commit (২০২৬-০৮-১৭):** `flushOutbox()`-এ একটা ডিজাইন বাগ ঠিক
...
- **সমস্যা:** প্রতিটা outbox এন্ট্রি সিঙ্ক করার সময় error হলে কোড শুধু
  ২টা case আলাদা করত: "409 conflict" (এন্ট্রি বাদ দিয়ে বাকিগুলোয়
  এগিয়ে যাওয়া) বনাম "বাকি সবকিছু" (পুরো loop `break` করে থামিয়ে
  দেওয়া, "নেট এখনো ফেরেনি" ধরে নিয়ে)। কিন্তু একটা নির্দিষ্ট ফাইলের
  জন্যই স্থায়ী ব্যর্থতা হতে পারে (যেমন ফাইলটা এর মধ্যে remote-এ
  ডিলিট হয়ে গেছে — GitHub সম্ভবত 422 রিটার্ন করে, 409 না, তাই আগের
  চেকে ধরা পড়ত না) — এমন হলে সেটা কোনো ভুল ছাড়াই সিঙ্ক হতে পারত এমন
  বাকি সব এন্ট্রিকেও আটকে দিত।
- **ফিক্স:** `break`-এর বদলে `continue` — একটা এন্ট্রি ব্যর্থ হলে
  loop থামে না, বাকি এন্ট্রিতেও চেষ্টা চলতে থাকে। সত্যিই অফলাইন হলে
  সবগুলোই একই কারণে দ্রুত ব্যর্থ হবে (loop তখনও প্রায় একই সময়ে শেষ
  হয়), কিন্তু আসলে সিঙ্ক হতে পারত এমন এন্ট্রি আর অহেতুক আটকে থাকবে না।
- মক ডেটা দিয়ে (৩টা এন্ট্রি, মাঝেরটা ইচ্ছাকৃতভাবে ব্যর্থ) পুরনো বনাম
  নতুন আচরণ পাশাপাশি চালিয়ে যাচাই করা হয়েছে — পুরনো আচরণে ০টা এন্ট্রি
  সিঙ্ক হতো (বাগ প্রমাণিত), নতুনটায় ঠিক ২টা (সমস্যাযুক্তটা বাদে বাকি
  সব) সঠিকভাবে সিঙ্ক হয়েছে। `/tmp`-এ, committed না।
- `flushSave()`-এর নিজস্ব `finally` ব্লকে unconditional early
  cache/outbox write (`cache.setFile`+`cache.queueOutboxEntry`, PUT
...
- **সমস্যা:** `openDb()`-এর `dbPromise` (module-level singleton cache)
  একবার reject হলে চিরকালের জন্য সেই রিজেক্টেড promise-ই ফেরত দিত —
  কখনো রিসেট হতো না। IndexedDB ব্যর্থতা প্রায়ই transient (সাময়িক) হয়
  (অন্য ট্যাবে blocked থাকা, quota-র সাময়িক সমস্যা), কিন্তু একবার ব্যর্থ
  হলে সেশনের বাকি পুরো সময় সব cache ফাংশন স্থায়ীভাবে অকার্যকর থেকে
  যেত — **সবচেয়ে গুরুত্বপূর্ণ, অফলাইন edit queue করাও (`queueOutboxEntry`)
  বন্ধ হয়ে যেত**, মানে অফলাইনে করা এডিট সত্যিকারের ডেটা-লস হতে পারত,
  শুধু ধীরগতির অ্যাপ না।
- **আরেকটা সংশ্লিষ্ট ফাঁক:** `indexedDB.open()`-এর `onblocked` handler
  ছিল না — অন্য ট্যাবে পুরনো ভার্সনের কানেকশন খোলা থাকলে upgrade
  request চিরকাল ঝুলে থাকতে পারত (`onsuccess`/`onerror` কোনোটাই ফায়ার
  না করেই), `await openDb()` কল করা যেকোনো জায়গা অনির্দিষ্টকালের জন্য
  আটকে যেত।
- **ফিক্স:** ব্যর্থ হলে (`dbPromise.catch(() => { dbPromise = null; })`)
  রিসেট করে দেওয়া হচ্ছে, যাতে পরের কল আবার নতুন করে খোলার চেষ্টা করে।
  `onblocked` handler যোগ করা হয়েছে (৩ সেকেন্ড অপেক্ষা করে তারপরও
  আটকে থাকলে reject, network-only fallback-এ যেতে পারে)।
...
**commit (২০২৬-০৮-১৫):** "দুই মোডাল stack" বাগের মূল কারণ
...
**commit (২০২৬-০৮-১৫): আরেকটা "দুই মোডাল একসাথে খোলা থাকা" বাগ
...
**commit (২০২৬-০৮-১৫):** সব নোটের কনটেন্ট background prefetch
...
**commit (২০২৬-০৮-১৫):** ইউজারের নির্দেশে ("বাগ খুঁজ pin
...
**commit (২০২৬-০৮-১৪):** নোট খুললে কনটেন্টের নিচের অংশ কাটা
পড়ে যাওয়া বাগ ফিক্স — মোবাইল ও ডেস্কটপ দুই জায়গাতেই (স্ক্রিনশট দিয়ে
ইউজার নিশ্চিত করেছিলেন)। মূল কারণ: `.editor-area`-তে `min-height: 0`
মিসিং ছিল — flex-item ডিফল্টভাবে নিজের কনটেন্টের চেয়ে ছোট হতে রাজি হয়
না (classic flexbox quirk), তাই লম্বা কনটেন্টে `.editor-area` নিজেই
বেড়ে গিয়ে viewport-এর বাইরে চলে যেত, আর `.app`-এর `overflow:hidden`
সেটাকে হুট করে কেটে ফেলত — `.cm-host`-এর নিজস্ব স্ক্রল কখনো কাজ করার
সুযোগ পেত না। `style.css`-এ `.editor-area { min-height: 0; }` যোগ করে
ঠিক করা হয়েছে (`.cm-host`-এ আগে থেকেই ছিল, শুধু তার parent-এ মিসিং
ছিল)।

**commit (২০২৬-০৮-১৩):** ইউজারের অনুরোধে আরও ৩টা কোডিং বাগ
খুঁজে ঠিক করা হয়েছে (একটার পর একটা, খুঁজে-ফিক্স-করে প্যাটার্নে)।

1. **Ctrl+K modal-stacking:** `openQuickSwitcher()`-এ অন্য কোনো modal
   (rename/new-file, settings, delete-confirm) আগে থেকে খোলা কিনা চেক
   ছিল না — Ctrl+K চাপলে তার উপর Quick Switcher খুলে দুটো ফুল-স্ক্রিন
   overlay stack হয়ে যেত। এখন `openQuickSwitcher()`-এ guard যোগ করা
   হয়েছে (অন্য modal খোলা থাকলে no-op)।
2. **`moveNode()`-এ duplicate ফাইলের ঝুঁকি:** নতুন জায়গায় PUT সফল
   হওয়ার পর পুরনো জায়গার DELETE ব্যর্থ হলে ফাইল দুই জায়গাতেই থেকে
   যেত, কোনো সতর্কতা ছাড়াই — আগে পুরো loop একটাই try/catch-এ থাকায়
   একটা ফাইল ব্যর্থ হলে বাকি (সফল হতে পারত এমন) ফাইলগুলোও থেমে যেত।
   এখন per-file error handling, delete ব্যর্থ হলে একবার retry, তারপরও
   ব্যর্থ হলে "duplicate, ম্যানুয়ালি মুছুন" স্পষ্টভাবে রিপোর্ট করে।
3. **`scripts/archive-notes.py`-এর নিজের বাগ:** পুরনো এন্ট্রি সরানোর
   পর label-সামঞ্জস্য ঠিক করার লজিক শুধু ২য়/৩য় এন্ট্রি "সর্বশেষ" বলছে
   কিনা ঠিক করত, কিন্তু প্রথম (রাখা) এন্ট্রিটা যদি কোনো কারণে ভুলবশত
   "তার আগের commit" দিয়ে শুরু হতো (যেমন কোনো ম্যানুয়াল conflict-resolve
   এর পর), সেটা প্রমোট করত না — ফলে archive-এর পর কোনো এন্ট্রিই
   "সর্বশেষ" না বলার সম্ভাবনা ছিল। মক ডেটা দিয়ে টেস্ট করে বাগটা
   reproduce করে, তারপর ফিক্স করে আবার টেস্ট করে (এবং স্বাভাবিক কেসে
   regression নেই তাও টেস্ট করে) নিশ্চিত হওয়া হয়েছে।

প্রতিটা fix `scripts/verify.sh` পাস করেছে; ১ ও ৩ নং আলাদা isolated টেস্ট
দিয়েও (মক ডেটা, `/tmp`-এ, committed না) যাচাই করা হয়েছে।
...
**commit (২০২৬-০৮-১৩):** "সর্বশেষ অবস্থা" সেকশন স্বয়ংক্রিয়ভাবে
ছোট রাখার সিস্টেম যোগ হয়েছে — এই সেকশন বড় হয়ে টোকেন-খরচ বাড়ছিল
(ইউজারের অভিযোগ), তাই ম্যানুয়াল মনে-রাখার বদলে এখন এটা `verify.sh`-এর
অংশ হিসেবে নিজে থেকেই চলে।
- নতুন `scripts/archive-notes.py`: এই সেকশনে `**সর্বশেষ commit (...):**`
  / `**তার আগের commit (...):**` প্যাটার্নে কতগুলো এন্ট্রি আছে গোনে,
  ৩টার বেশি হলে সবচেয়ে পুরনোগুলো `HISTORY.md`-এর "পুরনো সর্বশেষ অবস্থা"
  সেকশনের একদম উপরে (নতুন-থেকে-পুরনো ক্রম বজায় রেখে) সরিয়ে দেয়। label
